...
----------------------- */
// Display setup
DisplaySSD1306_128x32_I2C display(-1, { -1, 0x3C, WAND_SCL, WAND_SDA, 0 });
...
int barrelPeriod = 50;
TimerEvent hallCheckTimer;
int hallCheckPeriod = 1000;
...
void setup() {
	...
	Serial1.begin(9600, SERIAL_8N1, WAND_SERIAL_RX, WAND_SERIAL_TX);
	//Serial1.begin(9600);

	
	pinMode(WAND_RUMBLE_PIN, OUTPUT);
	pinMode(BRL_SENSE_PIN, INPUT);
	...
	hallCheckTimer.set(hallCheckPeriod, checkHall);
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Setup Complete");
	display.clear();
...
void checkHall() {
	long measure = analogRead(BRL_SENSE_PIN);
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Hall Sensor Reading:");
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, measure);
}
...
void loop() {
	...
	if (!packConnected) {
		if (((currentMillis - lastPackCheck) > 250)) {
			...
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Wand waiting for Pack ...");
	...
	if (firstLoop) {
		...
		if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "First loop in Wand");
...
void checkDisplayTimeout(unsigned long currentMillis) {
	if ((displayState != DISPLAY_OFF) && (currentMillis > lastDisplayUpdate) && (currentMillis - lastDisplayUpdate > displayIdleTimeout)) {
		if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Display timeout");
		display.clear();
...
void drawDisplay(unsigned long currentMillis) {
	...
	//if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Drawing Display: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, displayState);
	//if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Selected Index: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, selectedIndex);
	...
	switch (displayState){
		...
		case TRACK_DISPLAY:
			display.setFixedFont(ssd1306xled_font6x8);
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Displaying Track Name: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, trackList[trackNumber][1]);
		...
		case OVERHEAT_TIME_SET: {
			...
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, valStr);
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, secondsVal);
...
void updateInputs(unsigned long currentMillis) {
	...
	if (abs(newRotaryPosition - lastRotaryPosition) >= 4) {
		...
		if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Rotary Movement: " + movement);
...
void actSwitchToggle(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "ACT Switch: " + (actSwitch.on() ^ SW_WAND_ACT_INVERT));
...
void lowerSwitchToggle(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "LOWER Switch: " + (lowerSwitch.on() ^ SW_WAND_LO_INVERT));
...
void upperSwitchToggle(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "UPPER Switch: " + (upperSwitch.on() ^ SW_WAND_UP_INVERT));
...
void intButtonPress(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "INT Pressed");
...
void intButtonLongPress(void* ref) {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "INT Held");
}

void intButtonRelease(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "INT Released");
	...
	switch (state) {
		case MUSIC_MODE:
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Play/Pause SD Track");
...
void tipButtonPress(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Tip Pressed");
...
void tipButtonLongPress(void* ref) {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Tip Held");
}

void tipButtonRelease(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Tip Released");
...
void rotaryButtonPress(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Rotary Pressed");
}
...
void rotaryButtonLongPress(void* ref) {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Rotary Held");
}

void rotaryButtonRelease(void* ref) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Rotary Released");
	...
	switch (displayState){
		case TOP_MENU: {
			if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Selected Top Menu Item: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, selectedIndex);
...
void initialiseState(State newState, unsigned long currentMillis) {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Initialising State: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, (char) newState);
	...
	//if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Updating LEDs");
	...
	//if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Updating Pack State");
...
void stateUpdate(unsigned long currentMillis) {
	...
	switch (state) {
		...
		case FIRING:
			...
			analogWrite(WAND_RUMBLE_PIN, random(20, 200));
			break;
...
void onPackIonSwitch() {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Ion Switch Toggled: "); if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, toggle ? "ON" : "OFF");
}

void onPackEncoderButton() {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Encoder Pressed");
...
void onPackEncoderTurn() {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Encoder Turn");
	int movement = cmdMessenger.readInt32Arg();
	rotaryMove(millis(), movement);
}

void onPackSetTrack() {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Set Track");
	int newTrack = cmdMessenger.readInt16Arg();
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Requested Track Change To: %s\n" + *trackList[newTrack][1]);
...
void onPackSetVolume() {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Set Volume");
	int newVol = cmdMessenger.readInt16Arg();
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Requested Volume Change To: %i\n" + newVol);
...
void onUpdateMusicPlayingState() {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Music State Update");
	bool playingState = cmdMessenger.readBoolArg();
	musicPlaying = playingState;
}

void onPackConnect() {
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Pack Connection to Wand");
	packConnected = true;
	displayState = DISPLAY_OFF;
	drawDisplay(millis());
}
...
void onSendConfigToWand() {
	...
	if (DEBUG) cmdMessenger.sendCmd(eventSerialLog, "Received Config From Pack");
}
